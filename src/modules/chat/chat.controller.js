...
export const sendMessage = asyncHandler(async (req, res) => {
  ...
  const data = await chatService.sendMessage(req.user.id, req.params.id, req.body.body);
...
export const markRead = asyncHandler(async (req, res) => {
  ...
  const data = await chatService.markConversationRead(req.user.id, req.params.id);
  return sendSuccess(res, 200, "Conversation marked read", data);
});

export const getUnreadCount = asyncHandler(async (req, res) => {
  const data = await chatService.getUnreadCount(req.user.id);
